...
static inline void INIT_LIST_HEAD_VECTOR(struct list_head *list, int n)
{
    int i = 0;
    for (i = 0; i < n; ++i){
...
/**
 * Single-linked list. Added by Xie Han <e@pku.edu.cn>.
 * Saving one pointer each node. Useful when an addition pointer is
 * considered wasteful.
 */
...
static inline void INIT_SLIST_HEAD_VECTOR(struct slist_head *v, int n)
{
        int i;
        for ( i = 0; i < n; ++i){
                INIT_SLIST_HEAD(&v[i]);
...
static inline void slist_add_head(struct slist_node *new_entry,
...
static inline void slist_add_tail(struct slist_node *new_entry,
								  struct slist_head *list)
{
	new_entry->next = (struct slist_node *)0;
	if(list->last == &list->first)
	    slist_add_head(new_entry, list);
	else
	{
...
static inline void slist_add_after(struct slist_node *new_entry,
